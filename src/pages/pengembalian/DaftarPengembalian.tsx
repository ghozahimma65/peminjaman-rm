...
export function DaftarPengembalian() {
  ...
  return (
    <div className="min-h-full space-y-3 pb-8 text-slate-800">
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_2px_5px_rgba(15,23,42,0.08)]"><div className="flex items-center justify-between px-5 py-4"><h2 className="text-sm font-bold">Daftar Pengembalian</h2><span className="text-[10px] text-slate-500">Menampilkan {filteredData.length === 0 ? 0 : 1} - {filteredData.length} dari {filteredData.length} data</span></div><div className="overflow-x-auto"><table className="min-w-[1200px] w-full text-left text-[10px]"><thead className="bg-slate-50 text-[9px] uppercase text-slate-500"><tr><th className="px-4 py-3">No.</th><th className="px-4 py-3">Nomor</th><th className="px-3 py-3">Nama Pasien</th><th className="px-3 py-3">Asal Ruang</th><th className="px-3 py-3">Peminjam</th><th className="px-3 py-3">Tanggal Pinjam</th><th className="px-3 py-3">Tanggal Kembali</th><th className="px-3 py-3">Kondisi Berkas</th><th className="px-3 py-3">Keperluan</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Aksi</th></tr></thead><tbody className="divide-y divide-slate-100">{filteredData.length === 0 ? <tr><td colSpan={11} className="px-5 py-10 text-center text-slate-400">Belum ada data pengembalian.</td></tr> : filteredData.map((row, index) => { const late = isOverdue(row.tanggalBerkasKeluar, row.tanggalPinjam, row.tanggalBerkasKembali); return <tr key={row.id} className="h-14 hover:bg-slate-50"><td className="px-4">{index + 1}</td><td className="px-4 font-semibold text-emerald-700">{row.nomorRm}</td><td className="px-3">{row.namaPasien}</td><td className="px-3">{row.unit}</td><td className="px-3">{row.peminjamName}</td><td className="px-3">{formatDate(row.tanggalPinjam)}</td><td className="px-3">{formatDate(row.tanggalBerkasKembali)}</td><td className="px-3"><span className={`rounded-full px-2 py-1 text-[9px] font-semibold ${row.kondisiBerkas === "RUSAK" ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}>{row.kondisiBerkas === "RUSAK" ? "Tidak Lengkap" : "Lengkap"}</span></td><td className="px-3">{row.catatan || "-"}</td><td className="px-3"><span className={`rounded-full px-2 py-1 text-[9px] font-semibold ${late ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}>{late ? "Terlambat" : "Dikembalikan"}</span></td><td className="px-3"><div className="flex items-center gap-1.5"><button type="button" title="Edit kondisi" onClick={() => { setSelected(row); setEditCondition(row.kondisiBerkas || "BAIK"); setShowEdit(true); }} className="flex h-7 w-7 items-center justify-center rounded-md border border-yellow-200 bg-yellow-100 text-yellow-700 transition hover:bg-yellow-200 cursor-pointer"><Icons.Edit /></button><button type="button" title="Hapus" onClick={() => { setSelected(row); setShowDelete(true); }} className="flex h-7 w-7 items-center justify-center rounded-md border border-red-200 bg-red-100 text-red-600 transition hover:bg-red-200 cursor-pointer"><Icons.Trash /></button></div></td></tr>; })}</tbody></table></div><div className="flex items-center justify-between px-5 py-4 text-[10px] text-slate-500"><span>Total {filteredData.length} data</span><span>Halaman 1</span></div></section>
      <Modal
        isOpen={showEdit}
