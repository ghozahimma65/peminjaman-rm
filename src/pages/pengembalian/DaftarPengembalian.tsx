...
export function DaftarPengembalian() {
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      const history = await getPengembalianHistory({ tanggalMulai: startDate || undefined, tanggalAkhir: endDate || undefined, unit: unit || undefined });
      setData(history);
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-full space-y-3 pb-8 text-slate-800">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-[0_2px_5px_rgba(15,23,42,0.08)]"><div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-[1.2fr_1fr_1.4fr_1.1fr_auto]">
        ...
        <label className="text-[10px] font-semibold uppercase text-slate-600">Ruang<select value={unit} onChange={event => setUnit(event.target.value)} className="mt-1 h-9 w-full rounded-md border border-slate-300 bg-white px-3 text-xs font-normal normal-case"><option value="">Semua Ruang</option>{RUANGAN_OPTIONS.map(item => <option key={item} value={item}>{item}</option>)}</select></label>
