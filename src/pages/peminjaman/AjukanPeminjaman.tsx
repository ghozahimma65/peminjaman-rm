...
export function AjukanPeminjaman({ onNavigate, initialNomorRm = "" }: { onNavigate: (page: string) => void; initialNomorRm?: string }) {
  ...
  const handlePreview = (event: React.FormEvent) => {
    ...
    if (!unit) return setErrorMsg("Unit/Poli wajib diisi.");
    setShowPreview(true);
  };
      ...
      <div className="grid grid-cols-1 items-start gap-3 xl:grid-cols-[minmax(0,1.65fr)_minmax(260px,1fr)]">
        <form onSubmit={handlePreview} className="rounded-xl border border-slate-200 bg-white p-5 shadow-[0_2px_5px_rgba(15,23,42,0.1)]">
          ...
          <div className="mt-7 grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <label className={`${labelClass} md:col-span-2`}>Ruang Tujuan <span className="text-red-500">*</span><select value={unit} onChange={event => setUnit(event.target.value)} disabled={isLoading} className={inputClass}><option value="">Pilih unit tujuan</option><option value="Poli Umum">Poli Umum</option><option value="Poli Gigi">Poli Gigi</option><option value="IGD">IGD</option><option value="Rawat Inap">Rawat Inap</option></select></label>
          </div>
        ...
        <aside className="rounded-xl border border-slate-200 bg-white p-5 shadow-[0_2px_5px_rgba(15,23,42,0.1)]"><h2 className="border-b border-emerald-900/30 pb-2 text-xs font-semibold text-slate-800">DETAIL PEMINJAMAN</h2><dl className="mt-5 grid grid-cols-[90px_1fr] gap-y-3 text-[10px]"><dt className="text-slate-500">No. RM</dt><dd className="font-semibold">: {nomorRm || "-"}</dd><dt className="text-slate-500">Nama Pasien</dt><dd className="font-semibold">: {namaPasien || "-"}</dd><dt className="text-slate-500">Peminjam</dt><dd className="font-semibold">: {user?.name || "-"}</dd><dt className="text-slate-500">Ruang Tujuan</dt><dd className="font-semibold">: {unit || "-"}</dd><dt className="text-slate-500">Tanggal Pinjam</dt><dd className="font-semibold">: {tanggalPinjam ? new Date(tanggalPinjam).toLocaleString("id-ID") : "-"}</dd><dt className="text-slate-500">Jatuh Tempo</dt><dd className="font-semibold">: {tanggalKeluar ? new Date(tanggalKeluar).toLocaleString("id-ID") : "-"}</dd><dt className="text-slate-500">Jilid</dt><dd className="font-semibold">: {jilid || "-"}</dd><dt className="text-slate-500">Catatan / Keperluan</dt><dd className="font-semibold">: {catatan || "-"}</dd></dl><button type="button" onClick={() => { setNomorRm(""); setNamaPasien(""); setUnit(""); setJilid(""); setCatatan(""); }} className="mt-8 ml-auto block rounded-md bg-red-50 px-3 py-2 text-[10px] font-semibold text-red-600 hover:bg-red-100">⊗ Batalkan Peminjaman</button></aside>
      </div>

      <Modal isOpen={showPreview} onClose={() => setShowPreview(false)} onConfirm={() => void handleSave()} title="Ringkasan Peminjaman" confirmText="Simpan Peminjaman" cancelText="Kembali Edit" description={<div className="grid grid-cols-[120px_1fr] gap-3 text-left text-sm"><strong className="text-slate-500">Nomor RM</strong><span>: {nomorRm}</span><strong className="text-slate-500">Nama Pasien</strong><span>: {namaPasien}</span><strong className="text-slate-500">Peminjam</strong><span>: {user?.name}</span><strong className="text-slate-500">Unit / Poli</strong><span>: {unit}</span><strong className="text-slate-500">Tgl Pinjam</strong><span>: {new Date(tanggalPinjam).toLocaleString("id-ID")}</span><strong className="text-slate-500">Deadline</strong><span>: {tanggalKeluar ? new Date(tanggalKeluar).toLocaleString("id-ID") : "-"}</span></div>} />
    </div>
  );
}